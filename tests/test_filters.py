...
def test_accept_under_budget_and_warn_unknowns():
    d = deal()
    ok, reasons = validate_deal(d, Settings(_env_file=None, search_start_date=date(2026, 6, 1)), today=date(2026, 6, 1))
    assert ok
    assert reasons == []
...
def test_reject_over_budget():
    ok, reasons = validate_deal(deal(total_price=150), Settings(_env_file=None, search_start_date=date(2026, 6, 1)), today=date(2026, 6, 1))
    assert ok
    ok, reasons = validate_deal(deal(total_price=151), Settings(_env_file=None, search_start_date=date(2026, 6, 1)), today=date(2026, 6, 1))
    assert not ok
