...
- Tranche `021-serpapi-deals-zero-raw-fallback` archivee dans `docs/tasks/archive/021-serpapi-deals-zero-raw-fallback.md`.
...
Smoke reel NCE `2026-07-01,2026-08-31`, budget 150, nuits 1-7, mode flight: run `22`, 4 probes Deals appelees, HTTP 200 `Success`, top-level keys `search_metadata`, `search_parameters`, `departure_informations`, 0 offre brute partout, 0 offre affichee. Aucun faux resultat cree. Aucun sweep live large lancé. FlixBus live teste avec `RAPIDAPI_KEY`, mais RapidAPI retourne `403/429` (`You are not subscribed to this API` / `Too many requests`) et aucune offre actionnable n'est récupérée. Playwright reste squelette sûr désactivé par défaut.
...
Étape 03 - Si besoin produit, investiguer cote SerpApi/Google pourquoi Deals retourne `Success` sans liste exploitable sur NCE ete 2026, meme avec probes fallback.
